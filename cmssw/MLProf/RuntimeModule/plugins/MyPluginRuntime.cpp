/*
 * Example plugin to demonstrate the direct multi-threaded inference with
 * TensorFlow 2.
 */
...
class MyPluginRuntime
    : public edm::stream::EDAnalyzer<edm::GlobalCache<CacheData>> {
 public:
  ...
  static std::unique_ptr<CacheData> initializeGlobalCache(
...
std::unique_ptr<CacheData> MyPluginRuntime::initializeGlobalCache(
    const edm::ParameterSet& config) {
  // this method is supposed to create, initialize and return a CacheData
  // instance
...
void MyPluginRuntime::fillDescriptions(
...
MyPluginRuntime::MyPluginRuntime(const edm::ParameterSet& config,
                                 const CacheData* cacheData)
    : inputTensorNames_(
          config.getParameter<std::vector<std::string>>("inputTensorNames")),
      outputTensorNames_(
          config.getParameter<std::vector<std::string>>("outputTensorNames")),
...
std::tuple<float, float> mean_and_std(std::vector<float> data_vector) {
  ...
  for (float time : data_vector) {
    std_runtime += pow(time - mean_runtime, 2);
  }
  std_runtime /= runs;
...
void writeFileWholeVector(float batch_size, std::vector<float> runtimes,
                          std::string csv_file) {
...
float choose_value(std::string type, int incremental_value,
...
void MyPluginRuntime::analyze(const edm::Event& event,
  ...
  for (int batchSize : batchSizes_) {
    ...
    for (int input_class = 0; input_class < (int)inputLengths_.size();
         input_class++) {
      // definition and filling of the input tensors for the differents input
      // classes here with if statements -> 1D,2D,3D inputs (creating input
      // tensor with std::vector does not work.)
      if (inputLengths_[input_class] == 1) {
        input = tensorflow::Tensor(tensorflow::DT_FLOAT,
                                   {batchSize, inputSizes_[counter]});
        ...
        for (int b = 0; b < batchSize; b++) {
          for (int i = 0; i < inputSizes_[counter]; i++) {
            ...
            // float value = choose_value(inputType_, i, generator,
            // distribution); input.matrix<float>()(b, i) = value;
        ...
        for (int b = 0; b < batchSize; b++) {
          for (int i = 0; i < inputSizes_[counter]; i++) {
            for (int j = 0; j < inputSizes_[counter + 1]; j++) {
              ...
              // float value = choose_value(inputType_, i, generator,
              // distribution); input.matrix<float>()(b, i, j) = value;
          ...
          for (int i = 0; i < inputSizes_[counter]; i++) {
            for (int j = 0; j < inputSizes_[counter + 1]; j++) {
              for (int k = 0; k < inputSizes_[counter + 2]; k++) {
                ...
                // float value = choose_value(inputType_, i+j+k, generator,
                // distribution);
        ...
        std::cout << "The inputs must be one-, two- or three dimensional, an "
                     "error will be thrown"
                  << std::endl;
        throw std::invalid_argument(
            "input " + std::to_string(input_class) + " with name " +
            inputTensorNames_[input_class] + " has not the right dimension");
      }
    ...
    // from cmssw source code: inputs in run function is a vector of pairs
    // consisting of a string and a tensor
    std::vector<std::pair<std::string, tensorflow::Tensor>> inputs;
    for (int input_class = 0; input_class < (int)inputLengths_.size();
         input_class++) {
      inputs.push_back(
          {inputTensorNames_[input_class], input_classes_vector[input_class]});
    }
